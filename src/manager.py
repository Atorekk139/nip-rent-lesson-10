...
class Manager:
    ...
    parameters : Parameters
        Configuration parameters for the system (paths, limits).
    apartments : dict
        Storage for Apartment objects indexed by their keys.
    tenants : dict
        Storage for Tenant objects.
    transfers : list
        Historical list of all financial transfers.
    bills : list
        List of all costs and bills associated with apartments.
    ...
    def calculate_tax(self, year: int, month: int, tax_rate: float) -> float:
        """Calculate the tax amount based on the total income.

        Args:
        ----
            year (int): Settlement year.
            month (int): Settlement month (1-12).
            tax_rate (float): The tax rate to apply (e.g., 0.085).
